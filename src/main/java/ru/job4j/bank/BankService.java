 ...
 * и позволяет быстро находить необходимую информацию, в т.ч. о передвижении денежных средств между счетами клиентов
...
public class BankService {
    ...
    /**
     * создаём коллекцию под названием users типа HashMap, где будут храниться данные типа User (т.е. список клиентов);
    ...
    /**
     * Метод принимает на вход значение типа String - паспортные данные (чтобы по ним найти пользователя,
     * которому принадлежат эти паспортные данные), а также значение типа Account - банковский счёт,
     * который необходимо добавить конкретному пользователю (у одного пользователя может быть несколько счетов);
     * Найдя пользователя, с помощью if задаём условие, что пользователь имеет полные данные (не равен null);
     ...
     * Для этого мы перебираем коллекцию users циклом foreach, и проверяем на соответствие входящее значение
     * passport каждому элементу коллекции(нас интересует только одно поле пользователя - паспорт);
    ...
    /**
     * Метод ищет банковский счёт пользователя по паспорту пользователя и по реквизиту банковского счёта
     * @param passport паспорт пользователя User
     * @param requisite реквизит банковского счёта Account
     * @return account банковский счёт пользователя (у него их может быть несколько, нам нужен конкретный по реквизиту)
     */
